...
const CAT_COLORS = { Restaurant: "#E65100", Grocery: "#2E7D32", Professional: "#1565C0", Temple: "#BF360C", Community: "#6A1B9A", Beauty: COLORS.primary, Wellness: "#00796B", Kids: "#E64A19", Wedding: COLORS.primary };
  ...
  return (
    <div style={{ background: COLORS.bg, minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: "960px", margin: "0 auto", padding: "32px 20px 0" }}>
        <h1 style={{ fontFamily: ff, fontSize: "clamp(28px, 5vw, 36px)", fontWeight: 700, margin: "0 0 4px", color: COLORS.text }}>News</h1>
        <p style={{ fontSize: "14px", color: COLORS.textSecondary, margin: "0 0 20px" }}>Stories, spotlights, and updates from the community.</p>
      ...
      <div style={{ maxWidth: "960px", margin: "0 auto", padding: "0 20px 60px" }}>
          ...
          <div>
            {SPOTLIGHTS.map(s => (
              <Link key={s.slug} href={`/spotlight/${s.slug}`} style={{ textDecoration: "none", color: "inherit", display: "block", marginBottom: "20px" }}>
                <div style={{ background: COLORS.surface, borderRadius: "10px", overflow: "hidden", border: `1px solid ${COLORS.border}`, transition: "box-shadow 0.2s" }}>
                  <div style={{ width: "100%", height: "280px", overflow: "hidden" }}>
                    <img src={s.image} alt={s.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  </div>
                  <div style={{ padding: "24px" }}>
                    <div style={{ fontSize: "12px", color: COLORS.textSecondary, marginBottom: "8px" }}>{s.date} · {s.business} — {s.city}</div>
                    <h2 style={{ fontFamily: ff, fontSize: "24px", fontWeight: 700, margin: "0 0 8px", color: COLORS.text, lineHeight: 1.3 }}>{s.title}</h2>
                    <p style={{ fontSize: "15px", color: COLORS.textSecondary, margin: "0 0 12px", lineHeight: 1.6 }}>{s.subtitle}</p>
                    <span style={{ fontSize: "14px", fontWeight: 600, color: COLORS.primary }}>Read the full story →</span>
                  </div>
                </div>
              </Link>
            ))}
            <p style={{ fontSize: "13px", color: COLORS.textSecondary, textAlign: "center" }}>New spotlight every week · <Link href="/suggest" style={{ color: COLORS.primary, fontWeight: 600, textDecoration: "none" }}>Nominate a business →</Link></p>
          </div>
        )}
        ...
        {tab === "community" && (
          <div style={{ textAlign: "center", padding: "48px 20px" }}>
            ...
            <h2 style={{ fontFamily: ff, fontSize: "28px", fontWeight: 700, margin: "0 0 8px", color: COLORS.text }}>Community Spotlight</h2>
            <p style={{ fontSize: "16px", color: COLORS.textSecondary, margin: "0 0 24px", maxWidth: "520px", marginLeft: "auto", marginRight: "auto", lineHeight: 1.6 }}>
              Every community has unsung heroes. We want to tell their stories.
            </p>
            <p style={{ fontSize: "14px", color: COLORS.textSecondary, margin: "0 0 28px", maxWidth: "480px", marginLeft: "auto", marginRight: "auto", lineHeight: 1.6 }}>
        ...
        {tab === "new" && (
          <div>
            ...
            <Link href="/restaurants/honest-restaurant-troy" style={{ textDecoration: "none", color: "inherit", display: "block", marginBottom: "24px" }}>
              <div style={{ background: COLORS.surface, borderRadius: "10px", border: `2px solid ${COLORS.accent}30`, padding: "24px", transition: "box-shadow 0.2s" }}>
                <span style={{ display: "inline-block", padding: "3px 10px", borderRadius: "999px", fontSize: "10px", fontWeight: 700, background: `${COLORS.accent}15`, color: COLORS.accent, marginBottom: "10px" }}>FEATURED NEW ARRIVAL</span>
                <h3 style={{ fontFamily: ff, fontSize: "20px", fontWeight: 600, margin: "0 0 6px", color: COLORS.text }}>Honest Restaurant</h3>
                <div style={{ fontSize: "12px", color: COLORS.textSecondary, marginBottom: "8px", display: "flex", gap: "8px", alignItems: "center" }}>
                  ...
                  <span><Star size={11} fill={COLORS.accent} color={COLORS.accent} style={{ display: "inline", verticalAlign: "middle" }} /> 4.4</span>
                </div>
            ...
            {/* Recent listings */}
            <h3 style={{ fontFamily: ff, fontSize: "18px", fontWeight: 700, margin: "0 0 14px", color: COLORS.text }}>Recently Added</h3>
            <div style={{ display: "grid", gap: "10px" }}>
              {newListings.map(biz => (
                <Link key={biz.id} href={biz.slug ? `/restaurants/${biz.slug}` : "/businesses?cat=food"} style={{ textDecoration: "none", color: "inherit" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "14px", padding: "14px 18px", background: COLORS.surface, borderRadius: "10px", border: `1px solid ${COLORS.border}`, transition: "box-shadow 0.2s" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontFamily: fb, fontSize: "14px", fontWeight: 600, color: COLORS.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{biz.name}</div>
                      <div style={{ fontSize: "11px", color: COLORS.textSecondary, display: "flex", gap: "6px", alignItems: "center" }}>
                        ...
                        {biz.rating && <span><Star size={9} fill={COLORS.accent} color={COLORS.accent} style={{ display: "inline", verticalAlign: "middle" }} /> {biz.rating}</span>}
                      </div>
                    </div>
                  </div>
                </Link>
              ...
              <div style={{ display: "grid", gap: "16px" }}>
                {recaps.map(r => (
                  <Link key={r.id} href={`/news/recaps/${r.slug}`} style={{ textDecoration: "none", color: "inherit" }}>
                    <div style={{ background: COLORS.surface, borderRadius: "10px", padding: "24px", border: `1px solid ${COLORS.border}`, transition: "box-shadow 0.2s" }}>
                      <div style={{ fontSize: "12px", color: COLORS.textSecondary, marginBottom: "8px", display: "flex", gap: "8px", alignItems: "center" }}>
                        <Calendar size={12} />
                      ...
                      <h3 style={{ fontFamily: ff, fontSize: "20px", fontWeight: 600, margin: "0 0 8px", color: COLORS.text, lineHeight: 1.3 }}>{r.title}</h3>
                      {r.location && <div style={{ fontSize: "12px", color: COLORS.textSecondary, marginBottom: "8px" }}><MapPin size={11} style={{ display: "inline", verticalAlign: "middle" }} /> {r.location}</div>}
                      <p style={{ fontSize: "14px", color: COLORS.textSecondary, margin: "0 0 8px", lineHeight: 1.6 }}>{r.body_text?.substring(0, 200)}...</p>
