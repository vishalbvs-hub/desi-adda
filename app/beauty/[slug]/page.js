...
export async function generateMetadata({ params }) {
  ...
  const { data } = await supabase.from("beauty_brands").select("name, city, subcategories, rating, description").eq("slug", slug).single();
  if (!data) return { title: "Beauty Business Not Found — Desi Adda" };
  const sub = Array.isArray(data.subcategories) ? data.subcategories.join(", ") : "";
  return {
    title: `${data.name} — ${data.city} | Desi Adda`,
    description: `${data.name} in ${data.city}. ${sub || "Desi beauty salon"}. ${data.rating ? `Rated ${data.rating}/5.` : ""} ${data.description || ""}`.trim().substring(0, 160),
    openGraph: { title: `${data.name} — ${data.city}`, description: `${sub || "Desi beauty salon"} in ${data.city}, Metro Detroit.` },
  };
}

export default async function BeautyPage({ params }) {
  const { slug } = await params;
  const { data: biz } = await supabase.from("beauty_brands").select("*").eq("slug", slug).single();
  ...
  if (!biz) {
    return (
      <div style={{ minHeight: "60vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "12px" }}>
        ...
        <Link href="/businesses?cat=beauty" style={{ color: COLORS.primary, fontWeight: 600 }}>Browse beauty businesses</Link>
      </div>
    );
  }
    ...
    <>
      ...
      <div style={{ background: "#FFFBF5", minHeight: "100vh" }}>
        <div style={{ background: "white", borderBottom: "1px solid #EDE6DE", padding: "12px 20px" }}>
          <Link href="/businesses?cat=beauty" style={{ color: COLORS.primary, fontFamily: fb, fontWeight: 600, fontSize: "14px", textDecoration: "none" }}>← Back to Beauty</Link>
        </div>
        ...
        <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "28px 20px" }}>
          ...
          <div style={{ marginBottom: "24px" }}>
            <div style={{ display: "flex", gap: "8px", marginBottom: "8px", flexWrap: "wrap" }}>
              {subcats.map((cat, i) => <span key={i} style={{ padding: "3px 10px", borderRadius: "999px", fontSize: "11px", fontWeight: 600, background: "#FCE4EC", color: "#C2185B" }}>{cat}</span>)}
            </div>
          ...
          {/* Two-column layout */}
          <style>{`@media (min-width: 768px) { .beauty-grid { grid-template-columns: 1fr 320px !important; } }`}</style>
          <div className="beauty-grid" style={{ display: "grid", gridTemplateColumns: "1fr", gap: "20px" }}>
            {/* LEFT */}
            <div>
              ...
              {subcats.length > 0 && (
                <div style={{ background: "white", borderRadius: "16px", padding: "20px 24px", border: "1px solid #EDE6DE", marginBottom: "20px" }}>
                  <h3 style={{ fontFamily: ff, fontSize: "18px", fontWeight: 700, margin: "0 0 8px" }}>Services</h3>
