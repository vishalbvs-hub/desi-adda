  ...
  return (
    <div style={{ background: COLORS.bg, minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "32px 20px" }}>
        <h1 style={{ fontFamily: ff, fontSize: "clamp(28px, 5vw, 36px)", fontWeight: 700, margin: "0 0 4px", color: COLORS.text }}>Entertainment</h1>
        <p style={{ fontSize: "14px", color: COLORS.textSecondary, margin: "0 0 20px" }}>What to watch, what to listen to.</p>
        ...
        <div style={{ marginBottom: "20px" }}>
          <select value={langFilter} onChange={e => setLangFilter(e.target.value)} style={{
            padding: "8px 14px", borderRadius: "10px", border: `1px solid ${COLORS.border}`, fontSize: "13px", fontFamily: fb, color: COLORS.textSecondary, background: COLORS.surface, cursor: "pointer",
          }}>
        ...
        {tab === "watch" && (() => {
          ...
          <>
            <section style={{ marginBottom: "40px" }}>
              <h2 style={{ fontFamily: ff, fontSize: "20px", fontWeight: 600, margin: "0 0 16px", color: COLORS.text }}>New on OTT</h2>
            ...
            <section>
              <h2 style={{ fontFamily: ff, fontSize: "20px", fontWeight: 600, margin: "0 0 16px", color: COLORS.text }}>Recommendations</h2>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "14px" }}>
                {lists.map(l => (
                  <Link key={l.id} href={`/entertainment/lists/${l.slug}`} style={{ textDecoration: "none", color: "inherit" }}>
                    <div style={{ background: COLORS.surface, borderRadius: "10px", padding: "24px", border: `1px solid ${COLORS.border}`, borderLeft: `4px solid ${COLORS.accent}`, transition: "box-shadow 0.2s" }}
                      onMouseEnter={e => e.currentTarget.style.boxShadow = "0 4px 20px rgba(0,0,0,0.06)"}
                      onMouseLeave={e => e.currentTarget.style.boxShadow = "none"}>
                      <h3 style={{ fontFamily: ff, fontSize: "16px", fontWeight: 500, margin: "0 0 6px", color: COLORS.text }}>{l.title}</h3>
                      <p style={{ fontSize: "13px", color: COLORS.textSecondary, margin: "0 0 10px", lineHeight: 1.5 }}>{l.subtitle}</p>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: "12px", color: COLORS.textMuted }}>{l.film_count} films</span>
        ...
        {tab === "listen" && (() => {
          ...
          <>
            <section style={{ marginBottom: "40px" }}>
              <h2 style={{ fontFamily: ff, fontSize: "20px", fontWeight: 600, margin: "0 0 16px", color: COLORS.text }}>Curated Playlists</h2>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "14px" }}>
                {filterLang(playlists).map(p => {
                  const pc = PLATFORM_COLORS[p.platform] || "#333";
                  return (
                    <a key={p.id} href={p.playlist_url} target="_blank" rel="noopener noreferrer" style={{ background: COLORS.surface, borderRadius: "10px", padding: "20px", border: `1px solid ${COLORS.border}`, textDecoration: "none", color: "inherit", transition: "box-shadow 0.2s", display: "flex", flexDirection: "column", gap: "10px" }}
                      onMouseEnter={e => e.currentTarget.style.boxShadow = "0 4px 20px rgba(0,0,0,0.06)"}
                      onMouseLeave={e => e.currentTarget.style.boxShadow = "none"}>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span style={{ padding: "3px 10px", borderRadius: "999px", fontSize: "10px", fontWeight: 600, background: `${COLORS.accent}15`, color: COLORS.accent }}>{p.language}</span>
                        <span style={{ padding: "3px 10px", borderRadius: "999px", fontSize: "10px", fontWeight: 600, background: `${pc}15`, color: pc }}>{p.platform}</span>
                      </div>
                      <h3 style={{ fontFamily: ff, fontSize: "16px", fontWeight: 500, margin: 0, color: COLORS.text }}>{p.name}</h3>
                      <p style={{ fontSize: "12px", color: COLORS.textSecondary, margin: 0, lineHeight: 1.5 }}>{p.description}</p>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "auto" }}>
                        <span style={{ fontSize: "11px", color: COLORS.textMuted }}>{p.song_count} songs</span>
            ...
            <section>
              <h2 style={{ fontFamily: ff, fontSize: "20px", fontWeight: 600, margin: "0 0 16px", color: COLORS.text }}>Live Near You</h2>
              {musicEvents.length > 0 ? (
                <div style={{ display: "grid", gap: "10px" }}>
                  {musicEvents.map((ev, i) => {
                    const d = new Date(ev.event_date + "T00:00:00");
                    return (
                      <div key={`me-${ev.id}-${i}`} style={{ display: "flex", gap: "14px", padding: "14px 16px", background: COLORS.surface, borderRadius: "10px", border: `1px solid ${COLORS.border}`, alignItems: "center" }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontFamily: fb, fontSize: "14px", fontWeight: 600, color: COLORS.text }}>{ev.event_name}</div>
                ...
                <div style={{ textAlign: "center", padding: "40px 20px", background: COLORS.surface, borderRadius: "10px", border: `1px solid ${COLORS.border}` }}>
                  <p style={{ fontSize: "15px", color: COLORS.textSecondary, margin: "0 0 8px" }}>No upcoming music events. Check back soon!</p>
...
function MovieCard({ movie: m }) {
  ...
  return (
    <div style={{ background: COLORS.surface, borderRadius: "10px", overflow: "hidden", border: `1px solid ${COLORS.border}`, transition: "box-shadow 0.2s", display: "flex", flexDirection: "column" }}
      onMouseEnter={e => e.currentTarget.style.boxShadow = "0 4px 20px rgba(0,0,0,0.08)"}
      onMouseLeave={e => e.currentTarget.style.boxShadow = "none"}>
      <div style={{ width: "100%", aspectRatio: "2/3", overflow: "hidden", position: "relative", background: COLORS.bg }}>
      ...
      <div style={{ padding: "12px 14px", flex: 1, display: "flex", flexDirection: "column" }}>
        <h3 style={{ fontFamily: ff, fontSize: "14px", fontWeight: 500, margin: "0 0 6px", color: COLORS.text, lineHeight: 1.3 }}>{m.title}</h3>
        <div style={{ display: "flex", gap: "4px", flexWrap: "wrap", marginBottom: "8px" }}>
          {m.language && <span style={{ padding: "2px 6px", borderRadius: "999px", fontSize: "9px", fontWeight: 600, background: `${COLORS.accent}12`, color: COLORS.accent }}>{m.language}</span>}
        ...
        <div style={{ display: "flex", gap: "6px", marginTop: "auto" }}>
          ...
          <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(m.title + " official trailer")}`} target="_blank" rel="noopener noreferrer" style={{ flex: 1, padding: "7px 10px", borderRadius: "8px", background: COLORS.text, color: "white", fontSize: "11px", fontWeight: 700, fontFamily: fb, textDecoration: "none", textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center", gap: "3px" }}><Play size={10} /> Trailer</a>
        </div>
      </div>
    </div>
  );
