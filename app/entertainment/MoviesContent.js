      ...
      <div style={{ background: "white", borderBottom: "1px solid #EDE6DE", padding: "12px 20px" }}>
        <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
            {LANGUAGES.map(l => (<button key={l} onClick={() => setLang(l)} style={{ padding: "6px 14px", borderRadius: "999px", fontSize: "12px", fontFamily: fb, fontWeight: 600, cursor: "pointer", border: lang === l ? `2px solid ${SAFFRON}` : "2px solid #EDE6DE", background: lang === l ? SAFFRON : "white", color: lang === l ? "#2D2420" : COLORS.textMuted, transition: "all 0.25s" }}>{l}</button>))}
          </div>
...
function MovieCard({ movie, onTrailer, isOtt }) {
  ...
  return (
    <div style={{ borderRadius: "14px", overflow: "hidden", background: "white", border: "1px solid #EDE6DE", transition: "all 0.25s", display: "flex", flexDirection: "column" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.boxShadow = "0 8px 24px rgba(0,0,0,0.08)"; }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "none"; }}>
      <div style={{ position: "relative", height: "180px", background: "#2D2420", overflow: "hidden" }}>
        {movie.poster_url ? <img src={movie.poster_url} alt={movie.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "40px" }}>{"\u{1F3AC}"}</div>}
      ...
      <div style={{ padding: "14px", flex: 1, display: "flex", flexDirection: "column" }}>
        <h3 style={{ fontFamily: ff, fontSize: "15px", fontWeight: 700, margin: "0 0 6px", color: "#2D2420", lineHeight: 1.3 }}>{movie.title}</h3>
        ...
        <div style={{ display: "flex", gap: "6px", marginTop: "auto" }}>
          ...
          {movie.trailer_youtube_id && (<button onClick={() => onTrailer(movie.trailer_youtube_id)} style={{ flex: 1, padding: "8px 12px", borderRadius: "10px", background: "#2D2420", color: "white", fontSize: "11px", fontWeight: 700, fontFamily: fb, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "4px" }}><Play size={11} /> Trailer</button>)}
        </div>
      </div>
    </div>
  );
