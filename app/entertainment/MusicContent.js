      ...
      <div style={{ background: "white", borderBottom: "1px solid #EDE6DE", padding: "12px 20px" }}>
        <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: "6px" }}>
            {[{ id: "All", label: "All Platforms" }, { id: "Spotify", label: "Spotify", icon: SPOTIFY_ICON }, { id: "Apple Music", label: "Apple Music", icon: APPLE_ICON }].map(p => (<button key={p.id} onClick={() => setPlatform(p.id)} style={{ padding: "5px 12px", borderRadius: "999px", fontSize: "11px", fontFamily: fb, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: "4px", border: platform === p.id ? `2px solid ${SAFFRON}` : "2px solid #EDE6DE", background: platform === p.id ? SAFFRON : "white", color: platform === p.id ? "#2D2420" : COLORS.textMuted, transition: "all 0.25s" }}>{p.icon && p.icon} {p.label}</button>))}
          </div>
        </div>
      </div>
        ...
        <div style={{ flex: 1, minWidth: 0, padding: "36px 0" }}>
          ...
          {(section === "All" || section === "Songs") && filteredSongs.length > 0 && (
            <div style={{ marginBottom: "40px" }}>
              <h2 style={{ fontFamily: ff, fontSize: "clamp(22px, 3.5vw, 28px)", fontWeight: 700, margin: "0 0 6px", color: "#2D2420" }}>{"\u{1F525}"} Trending This Week</h2>
              <p style={{ fontSize: "12px", color: COLORS.textFaint, margin: "0 0 16px" }}>Updated weekly</p>
              <div style={{ display: "grid", gap: "8px" }}>
                {filteredSongs.map((song, i) => (
                  <div key={song.id || i} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 14px", background: "white", borderRadius: "12px", border: "1px solid #EDE6DE" }}>
                    <span style={{ fontFamily: ff, fontSize: "16px", fontWeight: 700, color: SAFFRON, width: "28px", textAlign: "center" }}>{song.rank || i + 1}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontFamily: fb, fontSize: "13px", fontWeight: 600, color: "#2D2420", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{song.title}</p>
          ...
          {(section === "All" || section === "Playlists") && filteredPlaylists.length > 0 && (
            <div style={{ marginBottom: "40px" }}>
              <h2 style={{ fontFamily: ff, fontSize: "clamp(22px, 3.5vw, 28px)", fontWeight: 700, margin: "0 0 16px", color: "#2D2420" }}>{"\u{1F3A7}"} Our Playlists</h2>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "14px" }}>
                {filteredPlaylists.map((pl, i) => (
                  <a key={pl.id || i} href={pl.url} target="_blank" rel="noopener noreferrer" style={{ padding: "16px", borderRadius: "14px", background: "white", border: "1px solid #EDE6DE", textDecoration: "none", color: "inherit", transition: "all 0.25s" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 6px 20px rgba(0,0,0,0.06)"; }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "none"; }}>
                    ...
                    <h4 style={{ fontFamily: ff, fontSize: "14px", fontWeight: 700, margin: "0 0 4px", color: "#2D2420" }}>{pl.name}</h4>
          ...
          {(section === "All" || section === "Live") && (detroitConcerts.length > 0 || midwestConcerts.length > 0) && (
            <div style={{ marginBottom: "40px" }}>
              <h2 style={{ fontFamily: ff, fontSize: "clamp(22px, 3.5vw, 28px)", fontWeight: 700, margin: "0 0 16px", color: "#2D2420" }}>{"\u{1F3A4}"} Live Near You</h2>
              <div style={{ display: "grid", gap: "12px" }}>
                {[...detroitConcerts, ...midwestConcerts].map((c, i) => {
                  const d = c.event_date ? new Date(c.event_date + "T00:00:00") : null;
                  const dateStr = d ? d.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "";
                  return (
                    <div key={c.id || i} style={{ display: "flex", gap: "14px", padding: "16px", background: "white", borderRadius: "14px", border: "1px solid #EDE6DE", alignItems: "center" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <h4 style={{ fontFamily: ff, fontSize: "15px", fontWeight: 700, margin: "0 0 3px", color: "#2D2420" }}>{c.name}</h4>
