  ...
  return (
    <div style={{ background: "#FFFBF5", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: "720px", margin: "0 auto", padding: "36px 20px 60px" }}>
        <h1 style={{ fontFamily: ff, fontSize: "clamp(28px, 5vw, 36px)", fontWeight: 700, margin: "0 0 8px", color: "#2D2420" }}>{list.title}</h1>
        {list.subtitle && <p style={{ fontSize: "16px", color: "#8A7968", margin: "0 0 8px", lineHeight: 1.5 }}>{list.subtitle}</p>}
        <p style={{ fontSize: "14px", color: "#A89888", margin: "0 0 32px" }}>{list.film_count} films</p>

        <div style={{ display: "grid", gap: "16px" }}>
          {films.map((film, i) => (
            <div key={i} style={{ display: "flex", gap: "16px", padding: "18px 20px", background: "white", borderRadius: "14px", border: "1px solid #EDE6DE", alignItems: "flex-start" }}>
              <div style={{ width: "36px", height: "36px", borderRadius: "10px", background: "#FFF3E0", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontFamily: ff, fontSize: "18px", fontWeight: 700, color: SAFFRON }}>{i + 1}</div>
              <div style={{ flex: 1 }}>
                <h3 style={{ fontFamily: ff, fontSize: "16px", fontWeight: 700, margin: "0 0 4px", color: "#2D2420" }}>{film.title} <span style={{ fontWeight: 400, color: "#8A7968", fontSize: "14px" }}>({film.year})</span></h3>
                ...
                {film.description && <p style={{ fontSize: "14px", color: "#5A4A3F", margin: 0, lineHeight: 1.5 }}>{film.description}</p>}
              </div>
            </div>
          ))}
        </div>
