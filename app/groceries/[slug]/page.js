  ...
  return (
    <>
      ...
      <div style={{ background: "#FFFBF5", minHeight: "100vh" }}>
        ...
        {photos.length > 0 && (
          <>
            <div style={{ display: "flex", gap: "4px", overflowX: "auto", background: "#2D2420", scrollSnapType: "x mandatory", WebkitOverflowScrolling: "touch", scrollbarWidth: "none" }}>
              {photos.map((url, i) => (
            ...
            {photos.length > 1 && <div style={{ background: "#2D2420", padding: "6px 0", textAlign: "center" }}><span style={{ fontSize: "11px", color: "rgba(255,255,255,0.4)", fontFamily: fb }}>{photos.length} photos — scroll →</span></div>}
          </>
        )}
        ...
        <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "28px 20px" }}>
          ...
          <div style={{ marginBottom: "24px" }}>
            ...
            <h1 style={{ fontFamily: ff, fontSize: "clamp(28px, 5vw, 40px)", fontWeight: 700, margin: "0 0 8px", color: "#2D2420" }}>{s.name}</h1>
            {s.rating && (
              <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "8px", fontSize: "14px" }}>
                ...
                <span style={{ color: "#8A7968" }}>({s.reviews?.toLocaleString()} reviews)</span>
              </div>
            )}
          ...
          <div className="grocery-grid" style={{ display: "grid", gridTemplateColumns: "1fr", gap: "20px" }}>
            ...
            <div>
              {/* What to Know */}
              {wtk && (
                <div style={{ background: "white", borderRadius: "16px", padding: "24px", border: "1px solid #EDE6DE", borderLeft: "4px solid #2E7D32", marginBottom: "20px" }}>
                  ...
                  {wtk.known_for && <div style={{ marginBottom: "14px" }}><div style={{ fontSize: "12px", fontWeight: 600, color: "#2E7D32", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "4px" }}>Known For</div><div style={{ fontSize: "15px", fontWeight: 600, color: "#2D2420" }}>{wtk.known_for}</div></div>}
                  {wtk.best_for && <div style={{ marginBottom: "14px" }}><div style={{ fontSize: "12px", fontWeight: 600, color: "#2E7D32", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "4px" }}>Best For</div><div style={{ fontSize: "14px", color: "#5A4A3F", lineHeight: 1.6 }}>{wtk.best_for}</div></div>}
                  {wtk.freshness_day && <div style={{ marginBottom: "14px" }}><div style={{ fontSize: "12px", fontWeight: 600, color: SAFFRON, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "4px" }}>Fresh Produce Day</div><div style={{ fontSize: "14px", color: "#5A4A3F" }}>{wtk.freshness_day}</div></div>}
                  {wtk.skip && <div style={{ marginBottom: "14px" }}><div style={{ fontSize: "12px", fontWeight: 600, color: "#C62828", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "4px" }}>Skip</div><div style={{ fontSize: "14px", color: "#5A4A3F" }}>{wtk.skip}</div></div>}
                  {wtk.pro_tip && <div style={{ padding: "12px 16px", background: "#FFFBF5", borderRadius: "10px", border: "1px dashed #E0D8CF" }}><div style={{ fontSize: "12px", fontWeight: 600, color: "#00796B", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "4px" }}>💡 Pro Tip</div><div style={{ fontSize: "14px", color: "#5A4A3F" }}>{wtk.pro_tip}</div></div>}
                </div>
              )}
              ...
              {s.specialties && (
                <div style={{ background: "white", borderRadius: "16px", padding: "20px 24px", border: "1px solid #EDE6DE", marginBottom: "20px" }}>
                  ...
                  <p style={{ fontSize: "14px", color: "#5A4A3F", margin: 0, lineHeight: 1.6 }}>{s.specialties}</p>
                </div>
              )}
                ...
                <div style={{ background: "white", borderRadius: "16px", padding: "18px 20px", border: "1px solid #EDE6DE", marginBottom: "16px" }}>
                  ...
                  <div style={{ display: "grid", gap: "2px", fontSize: "12px", color: "#5A4A3F" }}>
                    {s.hours.split(" | ").map((day, i) => {
                      ...
                      return (<div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", borderBottom: i < 6 ? "1px solid #F5EDE4" : "none" }}><span style={{ fontWeight: 600, fontSize: "11px" }}>{dayName}</span><span style={{ color: "#8A7968", fontSize: "11px" }}>{times.join(": ")}</span></div>);
                    })}
                  </div>
                </div>
              )}
