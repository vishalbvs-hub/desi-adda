          ...
          <div style={{ marginBottom: "24px" }}>
            {subs.length > 0 && (
              <div style={{ display: "flex", gap: "8px", marginBottom: "8px", flexWrap: "wrap" }}>
                {subs.map(s => <span key={s} style={{ padding: "3px 10px", borderRadius: "999px", fontSize: "11px", fontWeight: 600, background: "#FFF3E0", color: "#E65100" }}>{s}</span>)}
              </div>
            )}
            <h1 style={{ fontFamily: ff, fontSize: "clamp(28px, 5vw, 40px)", fontWeight: 700, margin: "0 0 8px", color: "#2D2420" }}>{t.name}</h1>
            {t.rating && (
              <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "8px", fontSize: "14px" }}>
                <div style={{ display: "flex", gap: "1px" }}>
                  {[1,2,3,4,5].map(i => <svg key={i} width="16" height="16" viewBox="0 0 24 24" fill={i <= Math.round(t.rating) ? SAFFRON : "#E0D8CF"} stroke="none"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>)}
                </div>
                <span style={{ fontWeight: 700, color: "#2D2420" }}>{t.rating}</span>
                {t.reviews && <span style={{ color: "#8A7968" }}>({t.reviews.toLocaleString()} reviews)</span>}
              </div>
            )}
            ...
            <div>
              {t.description && (
                <div style={{ background: "white", borderRadius: "16px", padding: "24px", border: "1px solid #EDE6DE", marginBottom: "20px" }}>
                  ...
                  <p style={{ fontSize: "15px", color: "#5A4A3F", margin: 0, lineHeight: 1.7 }}>{t.description}</p>
                </div>
              )}
