...
export async function generateMetadata({ params }) {
  ...
  const { data } = await supabase
    .from("temples")
    .select("name, city, subcategories, rating, description")
    .eq("slug", slug)
    .single();
  ...
  return {
    ...
    openGraph: {
      ...
      description: `${subs || "Temple"} in ${data.city}, Metro Detroit.`,
    },
  };
}
    ...
    <>
      ...
      <div style={{ background: "#FFFBF5", minHeight: "100vh" }}>
        ...
        <div style={{ background: "white", borderBottom: "1px solid #EDE6DE", padding: "12px 20px" }}>
          <Link href="/temples" style={{ color: COLORS.primary, fontFamily: fb, fontWeight: 600, fontSize: "14px", textDecoration: "none" }}>
            ← Back to Temples
          </Link>
        </div>
        ...
        <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "28px 20px" }}>
          ...
          <div style={{ marginBottom: "24px" }}>
            ...
            <h1 style={{
              fontFamily: ff, fontSize: "clamp(28px, 5vw, 40px)", fontWeight: 700,
              margin: "0 0 8px", color: "#2D2420",
            }}>{t.name}</h1>
            ...
            {t.rating && (
              <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "8px", fontSize: "14px" }}>
                <div style={{ display: "flex", gap: "1px" }}>
                  {[1, 2, 3, 4, 5].map((i) => (
                    <svg key={i} width="16" height="16" viewBox="0 0 24 24" fill={i <= Math.round(t.rating) ? SAFFRON : "#E0D8CF"} stroke="none">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "6px", fontSize: "14px", color: "#5A4A3F" }}>
              ...
              {t.phone && (
                <div>📞 <a href={`tel:${t.phone}`} style={{ color: COLORS.primary, textDecoration: "none" }}>{t.phone}</a></div>
              )}
              {t.url && (
                <div>🌐 <a
                ...
                >{t.url.replace(/^https?:\/\/(www\.)?/, "").replace(/\/$/, "")}</a></div>
              )}
            </div>
          </div>
            ...
            <div>
              {t.hours && (
                <div style={{
                  ...
                  border: "1px solid #EDE6DE", marginBottom: "16px",
                }}>
              ...
              {t.latitude && t.longitude && (
                <div style={{ marginBottom: "16px", borderRadius: "16px", overflow: "hidden", border: "1px solid #EDE6DE" }}>
                  <iframe
                    ...
                    src={`https://www.google.com/maps?q=${t.latitude},${t.longitude}&z=15&output=embed`}
                  />
                </div>
              )}
            </div>
