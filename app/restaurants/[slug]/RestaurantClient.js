...
function StarIcon({ filled, size = 14 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill={filled ? SAFFRON : "#E0D8CF"} stroke="none">
    ...
    <>
      ...
      {topQuotes.length > 0 && (
        <div style={{ marginBottom: "24px" }}>
          <h3 style={{ fontFamily: ff, fontSize: "20px", fontWeight: 700, margin: "0 0 16px", color: "#2D2420" }}>
            ...
            {topQuotes.map((rev, i) => {
              ...
              return (
                <div key={i} style={{
                  ...
                  border: "1px solid #EDE6DE", borderLeft: `3px solid ${SAFFRON}`,
                  position: "relative",
                }}>
                  <div style={{ fontSize: "32px", color: `${SAFFRON}30`, fontFamily: "Georgia", position: "absolute", top: "10px", left: "16px", lineHeight: 1 }}>"</div>
                  <p style={{ fontSize: "14px", color: "#5A4A3F", lineHeight: 1.6, margin: "8px 0 12px", fontStyle: "italic", paddingLeft: "8px" }}>
                    {(snippet || rev.review_text).substring(0, 200)}{(snippet || rev.review_text).length > 200 ? "..." : "."}
                  </p>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ fontSize: "12px", fontWeight: 600, color: "#8A7968" }}>— {rev.author_name?.split(" ")[0]}</span>
              ...
              <div key={cr.id} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    ...
                    <div>
                      <div style={{ fontFamily: fb, fontSize: "14px", fontWeight: 600, color: "#2D2420" }}>{cr.reviewer_name}</div>
                      {cr.hometown && (
                        <span style={{ fontSize: "11px", color: "#8A7968", display: "flex", alignItems: "center", gap: "3px" }}>
                ...
                <p style={{ fontSize: "13px", color: "#5A4A3F", lineHeight: 1.6, margin: 0 }}>{cr.review_text}</p>
              </div>
            ))}
          </div>
        ) : (
          <div style={{ padding: "32px", textAlign: "center", background: "white", borderRadius: "16px", border: "1px dashed #E0D8CF" }}>
            <p style={{ fontSize: "15px", color: "#8A7968", margin: "0 0 4px" }}>Be the first to review this restaurant!</p>
            <p style={{ fontSize: "12px", color: "#A89888", margin: 0 }}>Your review helps the community find great food.</p>
          </div>
        )}
      </div>

      {/* ═══ REVIEW FORM ═══ */}
      <div id="review-form" style={{ background: "white", borderRadius: "16px", padding: "24px", border: "1px solid #EDE6DE", marginBottom: "24px" }}>
        <h3 style={{ fontFamily: ff, fontSize: "20px", fontWeight: 700, margin: "0 0 4px", color: "#2D2420" }}>
          Share Your Experience
        </h3>
        <p style={{ fontSize: "13px", color: "#8A7968", margin: "0 0 20px" }}>
        ...
        {submitted ? (
          <div style={{ textAlign: "center", padding: "24px 0" }}>
            ...
            <p style={{ fontFamily: ff, fontSize: "18px", fontWeight: 600, margin: "0 0 4px", color: "#2D2420" }}>Thanks!</p>
            <p style={{ fontSize: "13px", color: "#8A7968", margin: 0 }}>Your review will appear after a quick check.</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} style={{ display: "grid", gap: "16px" }}>
            <div>
              <label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#5A4A3F", marginBottom: "6px" }}>Rating *</label>
              <StarSelector rating={formData.rating} onChange={v => setFormData({ ...formData, rating: v })} />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
              <div>
                <label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#5A4A3F", marginBottom: "6px" }}>Your Name *</label>
                <input value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="First name or nickname"
                  style={{ width: "100%", padding: "10px 14px", borderRadius: "10px", border: "1px solid #E0D8CF", fontSize: "14px", fontFamily: fb, outline: "none", boxSizing: "border-box" }} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#5A4A3F", marginBottom: "6px" }}>Where are you from?</label>
                <input value={formData.hometown} onChange={e => setFormData({ ...formData, hometown: e.target.value })} placeholder="e.g. Hyderabad, Chennai"
                  style={{ width: "100%", padding: "10px 14px", borderRadius: "10px", border: "1px solid #E0D8CF", fontSize: "14px", fontFamily: fb, outline: "none", boxSizing: "border-box" }} />
              </div>
            </div>
            <div>
              <label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#5A4A3F", marginBottom: "6px" }}>What did you order?</label>
            ...
            <div>
              <label style={{ display: "block", fontSize: "13px", fontWeight: 600, color: "#5A4A3F", marginBottom: "6px" }}>Your review * <span style={{ fontWeight: 400, color: "#A89888" }}>({500 - formData.text.length} chars left)</span></label>
              <textarea value={formData.text} onChange={e => { if (e.target.value.length <= 500) setFormData({ ...formData, text: e.target.value }); }} placeholder="What was great? What could be better?"
                rows={4} style={{ width: "100%", padding: "10px 14px", borderRadius: "10px", border: "1px solid #E0D8CF", fontSize: "14px", fontFamily: fb, outline: "none", boxSizing: "border-box", resize: "vertical" }} />
            </div>
      ...
      {googleReviews.length > 0 && (
        <div style={{ marginBottom: "24px" }}>
          <button onClick={() => setShowAllGoogle(!showAllGoogle)} style={{
            ...
            <span style={{ fontFamily: fb, fontSize: "14px", fontWeight: 600, color: "#8A7968" }}>
              {showAllGoogle ? "Hide" : "See all"} {googleReviews.length} Google reviews
            </span>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#8A7968" strokeWidth="2" strokeLinecap="round"
          ...
          {showAllGoogle && (
            <div style={{ display: "grid", gap: "10px" }}>
              {googleReviews.map((rev, i) => (
                <div key={rev.id || i} style={{ background: "white", borderRadius: "12px", padding: "14px 18px", border: "1px solid #EDE6DE" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                      ...
                      <span style={{ fontSize: "13px", fontWeight: 600, color: "#2D2420" }}>{rev.author_name}</span>
                      <span style={{ fontSize: "11px", color: "#A89888" }}>{rev.review_time}</span>
                    </div>
                    <div style={{ display: "flex", gap: "1px" }}>
                      {[1,2,3,4,5].map(s => <StarIcon key={s} filled={s <= (rev.rating || 0)} size={12} />)}
                    </div>
                  </div>
                  <p style={{ fontSize: "12px", color: "#5A4A3F", lineHeight: 1.5, margin: 0 }}>
