import {
  ...
  ShoppingBag, Church, Gem, Heart, PartyPopper, Stethoscope, Film, UserCheck,
} from "lucide-react";
...
export async function fetchEventHalls() {
  const { data, error } = await supabase.from("event_halls").select("*").order("name");
  if (error) { console.error("Error fetching event halls:", error); return []; }
  return data.map(r => ({ ...r, sub: r.subcategories }));
}
...
export async function fetchAllData() {
  const [
    ...
    classifieds, blogArticles, travelHomeland, professionals, eventHalls,
  ] = await Promise.all([
    ...
    fetchBlogArticles(), fetchTravelHomeland(), fetchProfessionals(), fetchEventHalls(),
  ]);

  // Build CATEGORIES with the same shape as before — each category has a `data` array
  const CATEGORIES = [
    { id: "weddings", name: "Weddings & Celebrations", count: String(weddingVendors.length) + "+", icon: Gem, color: "#2D5A3D", data: weddingVendors, subs: ["Photography", "Cinematography", "Mehndi", "DJ", "Entertainment", "Sangeet", "Planner", "Decor", "Mandap", "Makeup", "Catering"], desc: "From mehndi to mandap — every vendor for a desi wedding in Michigan." },
    ...
    { id: "event-halls", name: "Event Halls", count: String(eventHalls.length) + "+", icon: Home, color: "#795548", data: eventHalls, subs: ["Banquet Hall", "Wedding Venue", "Party Hall", "Conference"], desc: "Banquet halls and venues for weddings, receptions, and community events." },
    ...
    { id: "professionals", name: "Professionals", count: String(professionals.length) + "+", icon: UserCheck, color: "#37474F", data: professionals, subs: [], desc: "Doctors, attorneys, CPAs, realtors, and financial advisors." },
  ];
